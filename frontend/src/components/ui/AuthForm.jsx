...
export default function AuthForm({ mode }) {
  ...
  async function handleEmailSubmit(event) {
    ...
    try {
      ...
      // Log the full error for debugging (visible in browser DevTools Console)
      console.error('Auth error:', firebaseError?.code, firebaseError?.message, firebaseError);
      ...
      if (errorCode.includes('email-already')) {
        ...
        setError(`Something went wrong: ${firebaseError?.code || firebaseError?.message || 'Unknown error'}. Please try again.`);
      }
    } finally {
      setLoading(false);
    }
